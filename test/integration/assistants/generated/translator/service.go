...
func NewService(client aiwf.ModelClient) *Service {
	...
	// Initialize agents
	s.agents = &Agents{
		Translator: NewTranslatorAgent(client),
	}
	s.agents.Translator.Types = s // Inject TypeProvider
...
func (s *Service) GetInputTypeFor(agentName string) (string, any, error) {
	switch agentName {
	case "translator":
		return "TranslationRequest", TypeMetadata["TranslationRequest"], nil
	default:
...
func (s *Service) GetOutputTypeFor(agentName string) (string, any, error) {
	switch agentName {
	case "translator":
		return "TranslationResult", TypeMetadata["TranslationResult"], nil
	default:
		return "", nil, fmt.Errorf("agent %s not found", agentName)
	}
}
